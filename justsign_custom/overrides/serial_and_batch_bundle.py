...
class CustomSerialandBatchBundle(SerialandBatchBundle):
	def validate_quantity(self, row, qty_field=None):
		...
		total_qty = abs(flt(self.total_qty, precision))
		comparable_quantities = self.get_comparable_row_quantities(row, qty_field)

		if not any(abs(total_qty - abs(flt(qty, precision))) <= 0.01 for qty in comparable_quantities):
	...
	def get_comparable_row_quantities(self, row, qty_field):
		qty = row.get(qty_field)
		quantities = [qty]

		if qty_field == "qty" and row.get("stock_qty"):
			quantities.append(row.get("stock_qty"))
		...
		if (
			...
			quantities = [flt(qty) * flt(row.get("conversion_factor"))]

		return [value for value in quantities if value is not None]
